...
# Internal Imports
from polyscape.api.train import reinforce, reinforce_with_baseline, ppo
...
def add_arguments(parser: ArgumentParser):
    ...
    add_ppo_arguments(algos.add_parser("ppo"))
...
def add_ppo_arguments(parser: ArgumentParser):
    parser.set_defaults(
        train_fn_factory=lambda args: partial(
            ...
            epochs_per_trajectory=args.epochs_per_trajectory,
            clipping_epsilon=args.clipping_epsilon,
            discount_factor=args.discount_factor,
            gae_factor=args.gae_factor,
            value_fn_factor=args.value_fn_factor,
            entropy_fn_factor=args.entropy_fn_factor,
        )
    )

    def add_args(p: ArgumentParser):
        algo = p.add_argument_group("PPO Hyperparameters")
        algo.add_argument("--total-timesteps", type=int, default=10000)
        algo.add_argument("--horizon-timesteps", type=int, default=64)
        algo.add_argument("--epochs-per-trajectory", type=int, default=3)
        algo.add_argument("--clipping-epsilon", type=float, default=0.2)
        algo.add_argument("--discount-factor", type=float, default=0.99)
        algo.add_argument("--gae-factor", type=float, default=0.95)
        algo.add_argument("--value-fn-factor", type=float, default=1.0)
        algo.add_argument("--entropy-fn-factor", type=float, default=0.0)
